...
public class ClubEventAdapter extends RecyclerView.Adapter<ClubEventAdapter.EventViewHolder> {
    ...
    public void onBindViewHolder(final EventViewHolder holder, int position) {
        ...
        if(clubEvent.getImage_id()!=null&&!clubEvent.getImage_id().isEmpty()){
            Glide.with(context).load(clubEvent.getImage_id()).asBitmap().diskCacheStrategy(DiskCacheStrategy.ALL).error(R.drawable.news_intro).into(new ImageViewTarget<Bitmap>(holder.club_image) {
                @Override
