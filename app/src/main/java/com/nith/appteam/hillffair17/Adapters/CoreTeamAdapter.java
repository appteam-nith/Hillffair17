...
public class CoreTeamAdapter extends RecyclerView.Adapter<CoreTeamAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(final ViewHolder view_Holder, int i){
        ...
        if(!array_list.get(i).url.isEmpty()){
            Glide.with(context).load(array_list.get(i).url).asBitmap().diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.drawable.news_intro).error(R.drawable.news_intro).into(new ImageViewTarget<Bitmap>(view_Holder.imageView) {
                @Override
    ...
    public ViewHolder onCreateViewHolder(ViewGroup view_group, int i){
        View view= LayoutInflater.from(view_group.getContext()).inflate(R.layout.cardview_coreteam,view_group,false);
